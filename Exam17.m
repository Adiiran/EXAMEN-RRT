...
GetDistribution1::usage="Genera distribuci\[OAcute]n de llegadas tipo I";
GetDistribution2::usage="Genera distribuci\[OAcute]n de llegadas tipo II";
GetDistribution3::usage="Genera distribuci\[OAcute]n de llegadas tipo III";
GetDistribution4::usage="Genera distribuci\[OAcute]n de llegadas tipo IV";
GetDistribution5::usage="Genera distribuci\[OAcute]n de llegadas tipo V";
GetDistribution6::usage="Genera distribuci\[OAcute]n de llegadas tipo VI";

Begin["`private`"]

GetDistribution1[]:=Accumulate[Table[RandomVariate[ErlangDistribution[2,200]],{10000}]];
GetDistribution2[]:=Accumulate[Table[RandomVariate[ExponentialDistribution[100]],{10000}]];
GetDistribution3[]:=Accumulate[Table[RandomVariate[ParetoDistribution[.01,18]],{10000}]];

GetDistribution4[]:=Table[RandomVariate[ErlangDistribution[2,200]],{10000}];
GetDistribution5[]:=Table[RandomVariate[ExponentialDistribution[100]],{10000}];
GetDistribution6[]:=Table[RandomVariate[ParetoDistribution[.01,18]],{10000}];



Module[{tack=0.05,cback=9600,l=480,c=9600,tp=0.01,tw=0.0,ww=10,winMod=0,k=1/4,HiW=10/4 ,hiP=0.1,HiP=1/4,nSect=5},
SetIniParDraw[tP_,tAck_]:=(tp=tP;tack=tAck;);
GetIniParDraw[]:={tp,tack};
DrawLineInWin[u_,v_]:=
If[u[[1]]<=tw,
If[v[[1]]> tw,
...
If[XInWin[a=(arr[[1]]+(arr[[2]]+tp)/nSEC)],
Graphics[Text[GetTextPack[arr],{a,HiW/nSEC}]]
,Unevaluated[Sequence[]]
];

DrawManually[arr_]:=
Module[{xt=arr[[1]],xf=arr[[1]]+arr[[2]]},
If[arr[[4]]>=2,
...
DrawPacketTx[arr_]:=
Module[{xt=arr[[1]],tI=arr[[2]],x=arr[[1]]-tw},
If[x >= 0 ,
...
Map[(If[((tw-#[[1]]-#[[2]])>=If[#[[4]]>=2,tp/nSect,2 tp +tack ]) || (#[[1]]>tw+ww),Unevaluated[Sequence[]],#])&,arr];
]
End[]
EndPackage[]
